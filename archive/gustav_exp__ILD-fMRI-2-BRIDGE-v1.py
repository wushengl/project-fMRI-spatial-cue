...
def setup(exp):
    ...
    for i,di in enumerate(devs):
        ...
        if name.startswith(dev_name) and ch == dev_ch:  # dev_ch for BRIDGE, 8 for booth 1 computer
            dev_id = i,i,ch
    if dev_id:
    ...
    if do_loudness_match:
        ...
        for i in range(loudness_match_times): # minimum match times
            ...
            matched_levels = mri_tones.get_loudness_match(2016,matching_pool,dev_id[0],tone_level_start=exp.stim.ref_rms,do_addnoise=False,round_idx=i)
        ...
        while np.sum(conv_check)<len(conv_check): # stop when conv_check = len(conv_check), which means all 1
            ...
            # get new sample
            i+=1
            this_matched_levels = mri_tones.get_loudness_match(2016, list(this_matching_pool), dev_id[0],tone_level_start=exp.stim.ref_rms, do_addnoise=False,round_idx=i)
